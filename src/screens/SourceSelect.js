...
export default function SourceSelect({ navigation, route }) {
    ...
    const openGallery = async () => {
        try {
            ...
            navigation.navigate('CropSend', { device, deviceName, phase, imageUri: img.path });
        } catch (e) {
            if (e.code !== 'E_PICKER_CANCELLED') Alert.alert('Error', e.message);
        }
    };

    const openCamera = () => navigation.navigate('Camera', { device, deviceName, phase });
        ...
        <SafeAreaView style={s.container}>
            ...
            <View style={s.cards}>
                ...
                >
                    ...
                    <Text style={s.bigDesc}>Take a photo, then crop to 70×118 px</Text>
                </TouchableOpacity>

                <TouchableOpacity
                    style={[s.bigCard, s.cardGreen]}
                    onPress={openGallery}
                    activeOpacity={0.8}
                >
                    ...
                    <Text style={s.bigDesc}>Pick an existing photo and crop to 70×118 px</Text>
                </TouchableOpacity>
            </View>

            <Text style={s.hint}>
                Output: {ASSET_W}×{ASSET_H} px · RGB565 · 16,520 bytes
            </Text>
        </SafeAreaView>
    );
}
