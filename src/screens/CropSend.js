...
const TYPE_LABEL = {
    ...
    [IMAGE_TYPE.GREETING]: 'Greeting screen',
};

const FORMAT_LABEL = {
    [IMAGE_TYPE.PROFILE]: 'RGB565',
    [IMAGE_TYPE.LOGO]: 'RGB565',
    [IMAGE_TYPE.GREETING]: '1-bit mono',
};
...
export default function CropSend({ navigation, route }) {
    const {
        ...
        imageUri = null,
        greetingText = '',
        greetingBytes: greetingBytesArray = null,
    } = route.params;

    const { w, h } = getDimensions(imageType);
    const isGreeting = imageType === IMAGE_TYPE.GREETING;
    ...
    const convert = useCallback(async () => {
        ...
        try {
            if (isGreeting) {
                const bytes = new Uint8Array(greetingBytesArray);
                setImageBytes(bytes);
                setPreview(hexPreview(bytes, 20));
                addLog(`Greeting text: "${greetingText}"`);
                addLog(`Rendered as 1-bit mono: ${bytes.length} bytes`);
            } else {
                ...
                const bytes = await convertImageBuffer(buf, imageType);
                setImageBytes(bytes);
                setPreview(hexPreview(bytes, 20));

                addLog(`Converted to RGB565: ${bytes.length} bytes`);
            }

            addLog(`Protocol packets: ${TOTAL_PACKETS} × ${DATA_BYTES_PER_PACKET} bytes`);
            setStatus(STATUS.IDLE);
        } catch (err) {
            addLog('ERROR: ' + err.message);
            setStatus(STATUS.ERROR);
        }
    }, [imageUri, imageType, isGreeting, greetingBytesArray, greetingText]);
    ...
    const send = useCallback(async () => {
        ...
        try {
            await sendImage(device, imageBytes, imageType, (sent, total) => {
    ...
    // ── Navigate back to ImageTypeSelect to send another type ────
    const sendAnotherType = () => {
        navigation.navigate('ImageTypeSelect', { device, deviceName, phase });
    };
            ...
            <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
                ...
                <View style={s.previewCard}>
                    ...
                    <View style={s.previewRow}>
                        {isGreeting ? (
                            <View style={[s.greetingPreview, { width: w * 1.5, height: h * 0.75 }]}>
                                <Text style={s.greetingPreviewText} numberOfLines={4}>
                                    {greetingText}
                                </Text>
                            </View>
                        ) : (
                            <Image
                                source={{ uri: imageUri }}
                                style={[s.previewImg, { width: w * 1.7, height: h * 1.7 }]}
                                resizeMode="cover"
                            />
                        )}

                        <View style={s.infoCol}>
                            <InfoRow k="Type" v={TYPE_LABEL[imageType] || imageType} />
                            <InfoRow k="Size" v={`${w}×${h} px`} />
                            <InfoRow k="Format" v={FORMAT_LABEL[imageType] || 'RGB565'} />
                            <InfoRow k="Bytes" v={imageBytes ? imageBytes.length.toLocaleString() : '—'} />
                ...
                {status === STATUS.DONE && (
                    <View style={s.doneCard}>
                        ...
                        <View style={s.doneActions}>
                            {/* Send another type → back to ImageTypeSelect */}
                            <TouchableOpacity
                                onPress={sendAnotherType}
                                style={[s.doneBtn, s.doneBtnPrimary]}
                            >
                                <Text style={s.doneBtnPrimaryTxt}>Send logo / greeting</Text>
                            </TouchableOpacity>

                            {/* Start over → back to BleScanner */}
                            <TouchableOpacity
                                onPress={() => navigation.popToTop()}
                                style={[s.doneBtn, s.doneBtnSecondary]}
                            >
                                <Text style={s.doneBtnSecondaryTxt}>Start over</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                )}
...
const s = StyleSheet.create({
    ...
    previewImg: { borderRadius: 12, backgroundColor: '#f1f5f9', borderWidth: 1, borderColor: '#dbe3ee' },
    greetingPreview: {
    ...
    greetingPreviewText: {
        color: '#000000', fontSize: 10, textAlign: 'center', lineHeight: 15, fontFamily: 'monospace',
    },
    infoCol: { flex: 1, gap: 7 },
    ...
    statusBadge: {
        marginTop: 8, alignSelf: 'flex-start', backgroundColor: '#e2e8f0',
    ...
    progTrack: {
        height: 8, backgroundColor: '#e2e8f0', borderRadius: 999,
        overflow: 'hidden', marginBottom: 8,
    },
    progFill: { height: '100%', backgroundColor: '#22c55e', borderRadius: 999 },
    ...
    doneTxt: { fontSize: 18, fontWeight: '700', color: '#15803d', marginBottom: 16 },
    doneActions: {
